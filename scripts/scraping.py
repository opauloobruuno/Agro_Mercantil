...
# Configurações
URL_CONAB = "https://portaldeinformacoes.conab.gov.br/precos-agropecuarios-serie-historica.html"
PASTA_RAW = Path("raw")
...
def baixar_html(url: str) -> str:
    ...
    logger.info("HTML baixado com sucesso de %s (status: %s)", url, response.status_code)
...
def salvar_html(html_content: str):
    ...
    logger.info("HTML salvo em %s", ARQUIVO_HTML)
...
def extrair_tabelas(html_content: str) -> List[Dict]:
    ...
    for tabela in tabelas:
        cabecalhos = [th.get_text(strip=True).title() for th in tabela.find_all("th")]
        if not cabecalhos:
            cabecalhos = [f"coluna_{i+1}" for i in range(len(tabela.find_all("tr")[0].find_all("td")))]
        ...
        for linha in tabela.find_all("tr")[1:]:
            ...
            if len(celulas) == len(cabecalhos):
                ...
                if "commodity" in registro:
                    registro["commodity_padronizada"] = padronizar_categoria(
                        registro["commodity"],
                        categorias_validas=["Soja", "Milho", "Café Arábica", "Algodão", "Trigo"],
                    )
                if "regiao" in registro:
                    registro["regiao_padronizada"] = padronizar_categoria(
                        registro["regiao"],
                        categorias_validas=["Centro-Oeste", "Sudeste", "Sul", "Nordeste"],
                    )
                if "preco" in registro:
                    try:
                        ...
                        logger.warning("Preço inválido na linha: %s", registro["preco"])
                registros.append(registro)

    logger.info("Extraídos %s registros de %s tabelas.", len(registros), len(tabelas))
    return registros
...
def salvar_dados(registros: List[Dict]):
    ...
    logger.info("Dados salvos: %s registros em %s e %s", len(df), ARQUIVO_CSV, ARQUIVO_JSON)
